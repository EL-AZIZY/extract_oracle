...
Améliorations v2 :
  ...
  - Paramètres du pool configurables (min, max, increment)
"""
...
# ── Import des drivers disponibles ────────────────────────────────────────────
...
if not _DRIVERS:
    sys.exit(
        "  Aucun driver de base de données installé.\n"
...
class ConnectionPool:
    ...
    Une seule instance est créée et réutilisée sur toute la durée du programme.
    """
    ...
    def _init_pool(self) -> None:
        """Initialise le pool selon le type de base de données."""
        if self._db_type not in _DRIVERS:
            available = list(_DRIVERS.keys())
            sys.exit(
                f"❌  Driver '{self._db_type}' non installé ou non supporté.\n"
        ...
        log.info("Initialisation pool %s → %s@%s:%s/%s (min=%d max=%d)",
        ...
        log.info("Pool initialisé ✓")

    @contextmanager
    def acquire(self):
        """
        Context manager : acquiert une connexion du pool et la libère à la fin.
    ...
    def close(self) -> None:
        ...
        try:
            if self._db_type == "oracle":
                ...
                self._pool.closeall()
            # MySQL : pas de méthode closeall, les connexions expirent naturellement
            log.info("Pool de connexions fermé.")
        except Exception as e:
            log.warning("Erreur à la fermeture du pool : %s", e)
...
def _resolve_password(cfg: configparser.ConfigParser, section: str) -> str:
    """
    Résout le mot de passe avec priorité :
      1. Variable d'environnement référencée par 'password_env'
      2. Valeur directe 'password' dans le fichier .ini (déconseillé)
    """
    import os
    ...
    if env_var:
        ...
        if pw:
            ...
            return pw
        log.warning(
            "Variable d'environnement '%s' absente — tentative avec 'password' du fichier ini.",
            env_var,
        )
    ...
    if pw:
        log.warning(
            "⚠️  Mot de passe lu en clair depuis le fichier ini. "
            "Préférez 'password_env = NOM_VAR_ENV' pointant sur une variable d'environnement."
        )
        return pw

    sys.exit(
        "❌  Mot de passe introuvable. Définissez 'password_env = VOTRE_VAR' "
